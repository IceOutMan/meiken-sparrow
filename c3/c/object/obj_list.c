...
ObjList* newObjList(VM* vm, uint32_t elementNum){
    ...
    // 先分配内存，后调用 initObjHeder，避免gc无谓的遍历
    if(elementNum > 0 ){
...
// 在objlist中索引为index处插入value， 类似于 list[index]=value
...
static void shrinkList(VM* vm, ObjList* objList, uint32_t newCapacity){
    ...
    memManager(vm, objList->elements.datas, oldSize, nweSize);
    objList->elements.capacity = newCapacity;
}

// 删除list中索引为index处的与元素，即删除list[index]
