...
#define VALUE_TO_OBJTHREAD(value) ((ObjThread*)VALLUE_TO_OBJ(value))
...
#define VALUE_IF_NULL(value) ((value).type == VT_NULL)
...
// 原生方法指针
// *Primitive 是函数指针 bool (VM* vm, Value* args) 的函数原型
// Primitive primFn 就是声明了一个指向 bool (VM* vm, Value* args) 函数原型的函数指针
...
typedef struct {
    ...
    union{
        // 指向脚本方法所关联的C实现
        Primitive primFn;
        // 指向脚本代码编译后的 ObjClosure 或者 ObjFn
        ObjClosure* obj;
    };
} Method;
