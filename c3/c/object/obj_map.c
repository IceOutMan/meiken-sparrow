...
static uint32_t hashObj(ObjHeader *objHeader) {
    switch (objHeader->type) {
        ...
        default:
            RUN_ERROR("the hashable are objstring, obgrange and class.");
    }
    return 0;
}

// 根据 value 的类型调整相应的hash函数
...
// 在 entries 中添加 entry，如果是新的 key 则返回true
...
static Entry *findEntry(ObjMap *objMap, Value key) {
    ...
    while (true) {
        ...
        // key为VT_UNDEFINED且正为VT_TRUE表示探测链未断，可继续探测
        // key为VT_UNDEFINED且正为VT_FALSE表示探测链结束，探测结束
...
// 在objMap中实现key与value的官来呢：objMap[key] = value;
...
Value removeKey(VM *vm, ObjMap *objMap, Value key) {
    ...
    entry->value = VT_TO_VALUE(VT_TRUE); // 值为真，伪删
