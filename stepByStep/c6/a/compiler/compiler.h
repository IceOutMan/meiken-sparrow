...
typedef struct {
    // 如果此 upvalue 是直接外层函数的局部变量就置为 true, 否则置为 false
    bool isEnclosingLocalVar;

    // 外层函数中局部变量的索引或者外层函数中 upvalue 的索引
    // 这取决于 isEnclosingLocalVar 的值
    uint32_t index;
} Upvalue; // upvalue 结构
...
typedef struct {
    ...
    // 当其内层函数引用此变量时，由其内层函数来设置此项为 true
    bool isUpvalue;
...
typedef enum {
    ...
    SIGN_SUBSCRIPT_SETTER   // setter 形式的下标记
} SignatureType; // 方法的签名
