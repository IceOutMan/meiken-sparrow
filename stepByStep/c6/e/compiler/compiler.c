...
// 声明模块变量，与 defineModuleVar 的区别是不做重定义检查，默认声明
...
// 返回包含 cu->enclosingClassBK 的最近的CompileUnit
...
static void idMethodSignature(CompileUnit *cu, Signature *sign) {
    sign->type = SIGN_GETTER; // 刚识别到 id， 默认为 getter

    // new 方法为构造函数
    if (sign->length == 3 && memcmp(sign->name, "new", 3) == 0) {
        // 构造函数后面不能直接 = ， 既不能称为 setter
        if (matchToken(cu->curParser, TOKEN_ASSIGN)) {
        ...
        if (trySetter(cu, sign)) {
            // 若是 setter，此时已经将 type 改为了 setter ，直接返回
            return;
        }

        if (!matchToken(cu->curParser, TOKEN_LEFT_PAREN)) {
            // 若后面没有 ( 说明是 getter, type 已经在开头设置为 getter ，直接返回
            return;
        }

        // 至此 tpye 应该为一般形式的 SIGN_METHOD， 形式为 name(paralist)
...
static int findUpvalue(CompileUnit *cu, const char *name, uint32_t length) {
    ...
    if (directOuterLocalIndex != -1) {
        return addUpvalue(cu, false, (uint32_t) directOuterLocalIndex);
    }

    // 执行到此处 说明该 upvalue 对应的局部变量，返回-1
    return -1;
}

// 从局部变量和 upvalue 中查找符号 name
...
static void emitLoadOrStoreVariable(CompileUnit *cu, bool canAssign, Variable var) {
    if (canAssign && matchToken(cu->curParser, TOKEN_ASSIGN)) {
        ...
        emitStoreVariable(cu, var);     // 为 var 生成赋值指令
    } else {
        emitLoadVariable(cu, var); // 为 var 生成读取指令
    }
}

// 生成把实例对象 this 加载到栈的指令
...
static void compileBody(CompileUnit *cu, bool isConstruct) {
    ...
    compileBlock(cu);
    if (isConstruct) {
        // 若是构造函数 就加载 “this”对象 作为下面 OPCODE_RETURN 的返回值
        writeOpCodeByteOperand(cu, OPCODE_LOAD_LOCAL_VAR, 0);
    } else {
        // 否则 加载 null 占位
        writeOpCode(cu, OPCODE_PUSH_NULL);
    }
    // 返回编译结果，若是构造函数就返回 this，否则返回 null
    writeOpCode(cu, OPCODE_RETURN);
}

// 结束 cu 的编译工作，在其外层编译单元中为其创建闭包
#if DEBUG
static OjbFn* endCompileUnit(CompileUnit* cu, const char* debugName, uint32_t debugNameLen){
...
static ObjFn *endCompileUnit(CompileUnit *cu) {
    ...
    if (cu->enclosingUnit != NULL) {
        ...
        // 为 vm 在创建闭包时判断引用的是局部变量还是 upvalue
        // 下面为每个 upvalue 生成参数
        index = 0;
...
// 生成 getter 或 一般 method 调用指令
static void emitGetterMethodCall(CompileUnit *cu, Signature *sign, OpCode opCode) {
    Signature newSign;
    newSign.type = SIGN_GETTER; // 默认为 getter， 假设下面的两个 if 不执行
    newSign.name = sign->name;
    newSign.length = sign->length;
    newSign.argNum = 0;

    // 如果是method， 有可能有参数列表，在生成调用方法的指令前必须把参数入栈，否则运行方法时除了会获得错误的参数（既栈中已经有的数据）
    // 还会从方法中返回时，错误地回收参数空间而导致栈失衡
    ...
    if (matchToken(cu->curParser, TOKEN_LEFT_BRACE)) {
        newSign.argNum++;
        // 进入本 if 块时，上面的 if 块未必执行了
        // 此时 newSign.type 也许还是 GETTER，下面要将其设置为 METHOD
        newSign.type = SIGN_METHOD;
        CompileUnit fnCU;
...
#if DEBUG
        // 以此函数被传递给的方法来命名这个函数，函数名=方法名+" block arg"
        char fnName[MAX_SING_LEN + 10] = {'\0'};  // "block arg\0"
        uint32_t len = sign2String(&newSign, fnName);
        memmove(fnName + len, " block arg", 10);
        endCompileUint(&fnCU, fnName, len + 10);
#else
        endCompileUnit(&fnCU);
#endif
    }

    // 如果是在构造函数中调用了 super 则会执行到此，构造函数中调用的方法只能是 super
    ...
    // 根据签名生成调用指令，如果上面的三个 if 都未执行，此处就是 getter 调用
    emitCallBySignature(cu, &newSign, opCode);
}

// 生成方法调用指令，包括 getter 和 setter
