...
void prepareFrame(ObjThread* objThread, ObjClosure* objClosure, Value* stackStart){
    ...
    // thread 中的各个 frame 是共享 thread 的 stack
    // frame 用 frame->stackStart 指向各自 frame 在 thread->stack 中的起始地址
    frame->stackStart = stackStart; // 此处的参数 stackStart 表示在 Thread 中的 stack 中的占据的起始地址
...
void resetThread(ObjThread* objThread, ObjClosure* objClosure){
    ...
    ASSERT(objClosure != NULL, "objClosure isd NULL in function resetThread");
