...
#define ALLOCATE(vmPtr, type)\
    (type*)memManger(vmPtr, NULL, 0, sizeof(type))
...
#define DEALLOCATE(vmPtr, memPtr) memManger(vmPtr, memPtr, 0, 0)
    ...
    void type##BufferFillWrite(VM* vm, \
        ...
        if(newCounts > buf->capacity){\
            ...
            ASSERT(newSize > oldSize, "faint...memory allocate!");\
                buf->datas = (type*)memManage(vm, buf->datas. oldSize, newSize);\
        }\
        uint32_t cnt = 0;\
        while(cnt < fillCount){\
