...
static TokenType idOrkeyword(const char *start, uint32_t lenght) {
    uint32_t idx = 0;
    while (keywordsToken[idx].keyword != NULL) {
        if (keywordsToken[idx].length == lenght && \ 
                memcmp(keywordsToken[idx].keyword, start, lenght) == 0) {
...
static void getNextChar(Parser *parser) {
    // curChar = nextChar;  nextCharPtr++
    parser->curChar = *parser->nextCharPtr++;
}

// 查看下一个字符是否为期望的，如果是就读进来，返回 true，否则返回false
...
static void parseId(Parser *parser, TokenType type) {
    ...
    // abcd(M curChar is (,  nextCharPtr>M=5, curToken.start>a=0
...
static void parseUnicodeCodePoint(Parser *parser, ByteBuffer *buf) {
    ...
    // 为代码通用，下面直接写 buf->datas ,在此先写入byteNum个0，以保证事先有byteNum个空间
    ByteBufferFillWrite(parser->vm, buf, 0, byteNum);

    // 把 value 编码为 UTF-8后写入缓冲区buf
    ...
    while (true) {
        ...
        if (parser->curChar == '%') {
            ...
            if (parser->interpolationExpectRightParenNum > 0) {
                // 当前字符是%，插值NUM>0,说明在期待一个%( 的 )括号了，此时遇到了%说明嵌套了
...
static void skipComment(Parser *parser) {
    ...
    skipBlanks(parser); // 注释之后可能会有空白字符
}
...
// 若当前token为 expected 则读入下一个token 并返回 true
// 否则不读入token 且返回false
...
// 断言下一个token为expected，否则报错 errMsg
...
// 由于sourceCode 未必来自于文件file，有可能只是个字符串
// file 仅作为跟中待编译的代码的标识，方便报错 
