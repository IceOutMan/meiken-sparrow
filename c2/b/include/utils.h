...
void* memManger(VM* vm, void* ptr, uint32_t oldSize, uint32_t newSize);
...
#define DEALLOCATE_ARRAY(vmPtr, arrayPtr, count) \ 
        memManger(vmPtr, arrayPtr, sizeof(arrayPtr[0]) * count, 0)

#define DEALLOCATE(vmPtr, memPrt) memManager(vmPtr, memPtr, 0, 0)
...
#define DECLARE_BUFFER_TYPE(type)\
    ...
    void type##BuferInit(type##Buffer* buf);\
...
// 声明 String 的 Buffer， StringBUffer
...
// 声明 Int Char Byte 的Buffer， IntBuffer，ByteBuffer
...
#define IO_ERROR(...)\
        errorReport(NUMM, ERROR_IO,__VA_ARGS__)
