...
static void stringInterpolation(CompileUnit* cu, bool canAssign UNUSED) {
    ...
    // 会按照以下形式编译
    // ["a ", a+c, " d ", e, "f "].join()
    // 其中"a "和" d "是TOKEN_INTERPOLATION, a,c,e都是TOKEN_ID,"f "是TOKEN_STRING
...
//   2 解析内嵌表达式,如"a+c",将其结果添加到list
    do {
        // 1 处理TOKEN_INTERPOLATION中的字符串,如"a %(a+c)"中的"a "
        literal(cu, false);
        //将字符串添加到list
        emitCall(cu, 1, "addCore_(_)", 11);   //以_结尾的方法名是内部使用

        // 2 解析内嵌表达式,如"a %(a+c)"中的b+c
        expression(cu, BP_LOWEST);
        //将结果添加到list
        emitCall(cu, 1, "addCore_(_)", 11);
    } while (matchToken(cu->curParser, TOKEN_INTERPOLATION)); // 处理下一个内嵌表达式,如"a %(a+c) d %(e) f "中的" d %(e)"

    //读取最后的字符串,"a %(a+c) d %(e) f "中的"f "
