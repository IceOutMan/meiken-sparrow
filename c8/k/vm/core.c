...
static bool primMapSubscript(VM* vm, Value* args){
    ...
    // 从 MAP 中查找 key(args[1]) 对应的 value
...
static bool primMapSubscriptSetter(VM* vm, Value* args){
    ...
    RET_VALUE(args[2]); // 返回 value
}

// objMap.addCore_(_,_): 编译器编译 map 字面量时内部使用的
// 在map中添加 （key,value) 对儿，并返回 map自身
static bool primMapAddCore(VM* vm, Value* args){
    if(!validateKey(vm, args[1])){
        return false; // 出错了切换线程
    }
...
// objMap.iterate_(_): 迭代 map 中的entry
// 返回 entry 的索引供keyIteratorValue_ 和 valueIteratorValue_ 做迭代器
static bool primMapIterate(VM* vm, Value* args){
    ...
    while(index < objMap->capacity){
        ...
        if(!VALUE_IS_UNDEFINED(objMap->entries[index].key)){
            RET_NUM(index); // 返回 entry 索引
        }
        index++;
    }

    // 若没有有效的 entry了，就返回false，迭代结束
    RET_FALSE;
}
...
// System.importModule(_): 导入并编译模块 args[1] 把模块挂载到 vm->allModules
static bool primSystemImportModule(VM* vm, Value* args){
    if(!validateString(vm, args[1])){ // 模块名称为字符串
        return false;
    }

    // 导入模块 name 并编译， 把模块挂载到 vm->allModules
    ...
    nextThread->caller = vm->curThread;
    // 返回false， vm会切换到此加载模块的线程
    return false;
}

// System.getModuleVariable(_,_): 获取模块 args[1] 中的模块变量 args[2]
...
// System.writeString_(_): 输出字符串 args[1]
...
static char* readModule(const char* moduleName){
    ...
    return moduleCode; // 由主调函数将来 free 此空间
}
...
// 导入模块 moduleName，主要是编译模块加载到 vm->allModules
static Value importModule(VM* vm, Value moduleName){
    //  若已经导入则返回 NULL_VAL
...
// 在模块 moduleName 中获取模块变量 variableName
static Value getModuleVariable(VM* vm, Value moduleName, Value variableName){
    // 调用本函数前模块已经被加载了
    ...
    if(objModule == NULL){
        ...
        // 24是下面 sprintf 中 fmt 中除 %s 的字符个数
...
void buildCore(VM* vm){
    ...
    PRIM_METHOD_BIND(vm->mapClass, "iterate_()", primMapIterate);
    ...
    // 在核自举过程中创建了很多 ObjString 对象，创建过程中需要调用 initObjHeader 初始化对象头
    // 使其class指向vm->stringClass， 但那时的 vm->stringClass 尚未初始化成功，因此现在更正
