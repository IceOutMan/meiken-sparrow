...
void printfCurrentToken(Parser parser);
...
// 声明方法
static int declareMethod(CompileUnit* cu, char* signStr, uint32_t lenght){
    // 确保方法被录入到 vm->allMethodNames
    int index = ensureSymbolExist(cu->curParser->vm, &cu->curParser->vm->allMethodNames, signStr, lenght);

    // 下面判读方法是否定义， 即排除重定义
    IntBuffer* methods = cu->enclosingClassBK->inStatic ?
                                &cu->enclosingClassBK->staticMethods : &cu->enclosingClassBK->instantMethods ;
    ...
    // 若是新定义就加入，这里并不是注册新方法， 而是用索引来记录已经定义过的方法，用于以后排重复
    IntBufferAdd(cu->curParser->vm, methods, index);
    return index;
}

// 将方法 methodIndex 指代的方法装入 classVar 指代的 class.methods 中
static void defineMethod(CompileUnit* cu, Variable classVar, bool isStatic, int methodIndex){
    // 1. 待绑定的方法在调用本函数之前已放到栈顶
    // 2. 将方法所属的类加载到栈顶
    emitLoadVariable(cu, classVar);

    // 3. 生成OPCODE_STATIC_METHOD 或 OPCODE_INSTANCE_METHOD
    // 在运行时绑定
...
static void compileMethod(CompileUnit* cu, Variable classVar, bool isStatic){
    ...
    if(sign.type == SIGN_CONSTRUCT){
        ...
        uint32_t constructorIndex = ensureSymbolExist(cu->curParser->vm, &cu->curParser->vm->allMethodNames, signatureString, signLen);
...
static void compileClassDefinition(CompileUnit* cu){
    Variable classVar;
    if(cu->scopeDepth != -1){ // 目前只支持在模块作用于定义类
        COMPILE_ERROR(cu->curParser, "class definition must bu in the module scope");
    }
    ...
    }else{ // 默认加载 object 类为基类
        emitLoadModuleVar(cu, "object");
    }

    // 创建类需要知道域的个数，目前类未定义完成，因此域的个数未知
    // 因此先临时写255，待类编译完成后再回填属性数
    int fieldNumIndex = writeOpCodeByteOperand(cu, OPCODE_CREATE_CLASS, 255);

    // 虚拟机执行完成OPCODE_CREATE_CLASS 后， 栈顶留下了创建好的类
    // 因此现在可以用该类为之前声明的类名 className 赋值
    if(cu->scopeDepth == -1){
    ...
    while(!matchToken(cu->curParser, TOKEN_RIGHT_BRACE)){
        ...
        printfCurrentToken(*cu->curParser);
    ...
    // classBK.fields 的是由 compileVarDefinition 函数统计的
    ...
    // enclosingClassBK 用来表示是否在编译类
    // 编译完类之后要置空，编译下一个类时要重新赋值
    cu->enclosingClassBK = NULL;
...
static void compileFunctionDefinition(CompileUnit* cu){
    ...
    // 函数名称加上 Fn 前缀 作为模块变量存储
    char fnName[MAX_SIGN_LEN + 4] = {'\0'}; // " fn xxxx\0 "
    memmove(fnName, "Fn ", 3);
...
// import foo for bar1, bar2
    ...
    // 导入模块时的扩展名不需要，有可能用户会把模块的扩展名加上， 比如 import sparrow.sp ，这时候就要跳过扩展名
    ...
    // 1. 为调用 system.importModule("foo") 压入参数system
    ...
    // 3. 现在可以调用 system.importModule("foo")
    ...
    // 循环编译导入的模块变量（以逗号分隔），
    // 如 import foo for bar1, bar2 中的 bar1 bar2
    do{
        ...
        // 1. 为调用System.getModuleVariable("foo", "bar1") 压入 system
        emitLoadModuleVar(cu, "system");

        // 2. 为调用 System.getModuleVariable("foo","bar1") 压入 foo
        writeOpCodeShortOperand(cu, OPCODE_LOAD_CONSTANT, constModIdx);

        // 3. 为调用 System.getModuleVariable("foo", "bar1") 压入 bar1
...
ObjFn *compileModule(VM *vm, ObjModule *objModule, const char *moduleCode) {
    ...
    // 模块编译完成，生成 return null 返回, 避免执行下面 endCompileUnit 中添加的 OPCODE_END
    writeOpCode(&moduleCU, OPCODE_PUSH_NULL);
    writeOpCode(&moduleCU, OPCODE_RETURN);

    // 检查在函数 id 中用行号声明的模块变量是否在引用之后有定义
    ...
    while(idx < objModule->moduleVarValue.count){
        ...
        if(VALUE_IS_NUM(objModule->moduleVarValue.datas[idx])){
            ...
            ASSERT(str[objModule->moduleVarName.datas[idx].lenght] == '\0', "module var name is not closed!");
